...
arduino = serial.Serial('COM7', 9600) # Mudar o 'COM7' de acordo com a porta usb utilizada
time.sleep(2)
...
while True:
    ...
    # Temporizador para o envio do gesto
    if gesto_final != gesto_detectado:
    ...
    if tempo_inicial is not None:
        ...
        if tempo_decorrido >= tempo_limite:
            ...
            arduino.write(str(gesto_final).encode())
            print(f"Gesto enviado para o Arduino: {gesto_final}")
            tempo_inicial = None
            
            if tempo_inicial is None:
                
                contador_ok = time.time()
                
    if contador_ok is not None:
                
        if (time.time() - contador_ok) < 2:
                    
            cv2.putText(img,str("Ok"),(20,80), cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2)

    if arduino.in_waiting > 0:
        resposta_arduino = arduino.readline().decode().strip()
        print(f"Resposta do Arduino: {resposta_arduino}")


    cv2.putText(img, f"Tempo: {int(tempo_decorrido) if tempo_inicial else 0}s", (20, 50),cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
